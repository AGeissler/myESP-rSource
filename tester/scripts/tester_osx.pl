...
our $gTestXML;              # Object containing test XML file
...
sub GetBinInfo(){
   ...
   if ( ! $gTest_params{"compare_two_archives"} &&  `which stat ` !~ /no stat/ ){
     $gTest_params{"test_bin_mod_date"} = `stat -f %m $gTest_params{'test_binary'}`;
   }else{
   ...
   if ( $gRef_Test_params{"test_binary"} && $gTest_params{"compare_versions"} ){
     ...
     if ( `which stat ` !~ /no stat/ ){
      $gRef_Test_params{"test_bin_mod_date"} = `stat -f %m $gRef_Test_params{'test_binary'}`;
     }else{
...
sub invoke_tests($$$$$){
  ...
  foreach my $version (sort keys %bps_versions){
    # delete unnecessary files - may be lying around if previous test cycle
    # was interrupted
    delete_old_files();
    ...
    if ( ! $FileEnd ){ 
      ...
      if ( $line =~ /<\/configuration>/ ) { 
        ...
        if ( ! $XMLWildcardsFound ) {
          ...
          $InputXMLContents .= "  <enable_xml_wildcards>false</enable_xml_wildcards>\n";
        ...
        if ( ! $SummaryTagFound ) {
          ...
          $InputXMLContents .= "  <summary_variable>*</summary_variable>\n";
            ...
            # @ag150710
            # Write dictionary (temporary measure)
          $InputXMLContents .= "  <output_dictionary>true</output_dictionary>\n";
        }
