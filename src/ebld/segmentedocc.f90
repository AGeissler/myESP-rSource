...
!! Dynamic multi-segmented huaman thermal model
...
!! ESP-r tward the PhD research of Mohamad Rida.
      SUBROUTINE segmentedocc(HMETA,Top,RHOCC,ONB,CLO)

      COMMON/PERS/ISD1,ISM1,ISD2,ISM2,ISDS,ISDF,NTSTEP
      real headflux1,trunkflux1,larmflux1,rarmflux1,llegflux1
      real rlegflux1,H2Oexp1,H2Oswt1,Qoccsens1,Qocclat1,COflux
      real Tsko_av1,Tco_av1,Tsk_av1
      ...
      real rlegflux2,H2Oexp2,H2Oswt2,Qoccsens2,Qocclat2
      real Tsko_av2,Tco_av2,Tsk_av2
...
!     The huam body in this model is devided into 25 segment including ten fingers 
...
! the model is structured into types that each type has its parameters 
      type clothtype
            real Rth
            real Rev
            real thick
...
! parameters and variables linked to core as core capacitance, metabolic rate,mechanical work and temperature
      type coretype
            real CoreCap
            real coreMet
            real mechwork
...
! skin related: capacitance, metabolic, area, temperature,moisture
      type skintype
            real SkinCap
            real skinMet
            real SkinArea
...
! artery :radius, length and index
      type arterytype
            real radius
            real length
            integer index
...
! every type has unique value for each body segment
      type bodysegtype
            type (coretype) core
            type (skintype) skin
...
! ambient conditions are taken from ESP-r see later in the loop for each time step
      ...
      real :: sklattotal
      real :: Ca,ma,Taadj,time,v,arteryradius(138)
      real :: Cv,Cvs,mv,mvs,Tvsadj,Tvadj,mperfv
...
! read fabric properties!!
       ...
       line_index = 0
       count = 0
       open(299, file=auxFilename)
       do while (ios == 0)
         read(299, '(A)', iostat=ios) buffer
         if (ios == 0) then
...
! calculating the radius of the outer fabric layer
      do i=1,25
            if (bodyseg(i)%clothed==1) then
...
! the number of arteies and veins in each body segment
      bodyseg(HEAD)%n=27
      ...
      if (ONB==1.and.msegflag==1) then
      open(unit=92,file="initial_condition1.txt",status="UNKNOWN")
      do i=1,25
            read (92,*) (ic(i,j),j=1,4)
      end do
      ...
      close(unit=92)
      else if (ONB==2.and.msegflag==1) then
      open(unit=94,file="initial_condition2.txt",status="UNKNOWN")
      do i=1,25
            read (94,*) (ic(i,j),j=1,4)
      end do
            ...
            if (cfdcheck.GT.1) then
             if (ONB==1) then
              ...
              elseif(seg.GT.23)then
               Op_T=tavtrunk1 + 273
!               RH=RHOCC
              elseif(seg==1 .or. seg==3 .or.seg==5 .or.&
                     seg==7 .or. seg==9 .or. seg==11 .or. &
                     seg==13.or. seg==15)then
               Op_T=tavarml1 + 273
!               RH=RHOCC
              elseif(seg==2 .or. seg==4 .or.seg==6 .or. &
                     seg==8 .or. seg==10 .or. seg==12 .or.&
                   seg==14.or. seg==16)then
               Op_T=tavarmr1 + 273
!               RH=RHOCC
...
!               RH=RHOCC
            ...
            METAo=HMETA
                  ...
                  if (seg < 13) then
                      if (bodyseg(seg)%skin%OutSkTemp(2)<12 .and. bodyseg(seg)%skin%OutSkTemp(1)>12 &
                          .and. time>120 .and. Tcore_av(2)<=37.5) then
                          bodyseg(seg)%t_ava_local = time
                          bodyseg(seg)%AVAstart = bodyseg(seg)%AVABflow
                      else if (bodyseg(seg)%skin%OutSkTemp(2)>15 .and. bodyseg(seg)%skin%OutSkTemp(1)<=15 &
                          .and. time>120 .and. Tcore_av(2)<=37.5) then
                          bodyseg(seg)%t_ava_local = time
                          bodyseg(seg)%AVAstart = bodyseg(seg)%AVABflow
                      else if (bodyseg(seg)%skin%OutSkTemp(2)<33.2 .and. bodyseg(seg)%skin%OutSkTemp(1)>=33.2 &
                          .and. time>120 .and. Tcore_av(2)<=37.5) then
                          bodyseg(seg)%t_ava_local = time
                          bodyseg(seg)%AVAstart = bodyseg(seg)%AVABflow
                      else if (bodyseg(seg)%skin%OutSkTemp(2)>33.2 .and. bodyseg(seg)%skin%OutSkTemp(1)<=33.2 &
                          ...
                          bodyseg(seg)%t_ava_local = time
                          bodyseg(seg)%AVAstart = bodyseg(seg)%AVABflow
                      else if (bodyseg(seg)%skin%OutSkTemp(2)<33.2 .and. Tcore_av(2)<37.5 .and. Tcore_av(1)>=37.5) then
                          bodyseg(seg)%t_ava_local = time
                          bodyseg(seg)%AVAstart = bodyseg(seg)%AVABflow
...
! Here for each member the total perfusion rates and skin blood flows are calculated.
! each body part has its unique number of arteries.
! The total volumetric flow rate of blood through each artery member is calculated in (cm3/s)
                  ...
                  ma=ma+v*PI*arteries(4)%radius**2
                  ...
                  arteries(12)%mft=m
                  call calc_velocity(velocity,heartbeat,T_STP,time,v)
                  ma=ma+v*PI*arteries(12)%radius**2
                  ...
                  case (FOOT_L)
                      ...
                      perfusion=m*PI*arteries(127)%radius**2
                        ...
                        call four(ratios(128),heartbeat,T_STP,velocity,m,cardiacout)
                         arteries(128)%mft=m
                        call calc_velocity(velocity,heartbeat,T_STP,time,v)
                        ma=ma+v*PI*arteries(128)%radius**2
                  ...
                  case (FOOT_R)
                        ...
                        arteries(126)%mft=m
                        call calc_velocity(velocity,heartbeat,T_STP,time,v)
                        ma=ma+v*PI*arteries(126)%radius**2
            ...
            perfusion=bodyseg(seg)%mperf
              ...
              if (bodyseg(seg)%clothed==0) then
                  ...
                  select case (seg)
                  case (HEAD)
                        bodyseg(seg)%mv=perfusion
                  Tvadj=bodyseg(HEAD)%Tv(2)
                  case (U_CHEST)
                        mv=bodyseg(L_CHEST)%mv + bodyseg(UPPERARM_L)%mv + bodyseg(UPPERARM_R)%mv + bodyseg(HEAD)%mv &
                          + bodyseg(FOREARM_L)%mvs + bodyseg(FOREARM_R)%mvs
                  bodyseg(seg)%mv=mv+perfusion
                  if (mv>0) then
                      Tvadj=(bodyseg(HEAD)%Tv(2)*bodyseg(HEAD)%mv+(bodyseg(UPPERARM_L)%mv+ bodyseg(FOREARM_L)%mvs)*&
                          bodyseg(UPPERARM_L)%Tv(2)+(bodyseg(UPPERARM_R)%mv+ bodyseg(FOREARM_R)%mvs)*&
                          bodyseg(UPPERARM_R)%Tv(2)+bodyseg(L_CHEST)%mv*bodyseg(L_CHEST)%Tv(2))/mv
                  else
                      Tvadj=(bodyseg(HEAD)%Tv(2)+bodyseg(UPPERARM_L)%Tv(2)+bodyseg(UPPERARM_R)%Tv(2)+&
                      bodyseg(L_CHEST)%Tv(2))/4.0     
                  end if
                  case (L_CHEST)
                        mv=bodyseg(THIGH_L)%mv + bodyseg(THIGH_R)%mv
                         bodyseg(seg)%mv=mv+perfusion
                        Tvadj=(bodyseg(THIGH_L)%Tv(2)+bodyseg(THIGH_R)%Tv(2))/2.0
                  case (UPPERARM_L)
                        mv=bodyseg(FOREARM_L)%mv
                        bodyseg(seg)%mv=mv+perfusion
                        Tvadj=bodyseg(FOREARM_L)%Tv(2)
                  case (UPPERARM_R)
                        mv=bodyseg(FOREARM_R)%mv
                        bodyseg(seg)%mv=mv+perfusion
                        Tvadj=bodyseg(FOREARM_R)%Tv(2)
                  case (FOREARM_L)
                        mv=bodyseg(PALM_L)%mv
                        bodyseg(seg)%mv=mv+perfusion
                        Tvadj=bodyseg(PALM_L)%Tv(2)
                  case (FOREARM_R)
                        mv=bodyseg(PALM_R)%mv
                        bodyseg(seg)%mv=mv+perfusion
                        Tvadj=bodyseg(PALM_R)%Tv(2)
                  case (PALM_L)
                        mv=bodyseg(L_F1)%mv+bodyseg(L_F2)%mv+bodyseg(L_F3)%mv+bodyseg(L_F4)%mv+bodyseg(L_F5)%mv
                        bodyseg(seg)%mv=mv+perfusion
                        Tvadj=(bodyseg(L_F1)%Tv(2)+bodyseg(L_F2)%Tv(2)+bodyseg(L_F3)%Tv(2)+bodyseg(L_F4)%Tv(2)+&
                        bodyseg(L_F5)%Tv(2))/5
                  case (PALM_R)
                        mv=bodyseg(R_F1)%mv+bodyseg(R_F2)%mv+bodyseg(R_F3)%mv+bodyseg(R_F4)%mv+bodyseg(R_F5)%mv
                        bodyseg(seg)%mv=mv+perfusion
                        Tvadj=(bodyseg(R_F1)%Tv(2)+bodyseg(R_F2)%Tv(2)+bodyseg(R_F3)%Tv(2)+bodyseg(R_F4)%Tv(2)+&
                        bodyseg(R_F5)%Tv(2))/5
                  case (THIGH_L)
                        mv=bodyseg(CALF_L)%mv
                        bodyseg(seg)%mv=mv+perfusion
                        Tvadj=bodyseg(CALF_L)%Tv(2)
                  case (THIGH_R)
                        mv=bodyseg(CALF_R)%mv
                        bodyseg(seg)%mv=mv+perfusion
                        Tvadj=bodyseg(CALF_R)%Tv(2)
                  case (CALF_L)
                        mv=bodyseg(FOOT_L)%mv
                        bodyseg(seg)%mv=mv+perfusion
                        Tvadj=bodyseg(FOOT_L)%Tv(2)
                  case (CALF_R)
                        mv=bodyseg(FOOT_R)%mv
                        bodyseg(seg)%mv=mv+perfusion
              ...
              Cvs=bodyseg(seg)%Cbl_v
                  ...
                  case (HEAD)
                        ...
                        Tvsadj=0.0
                  case (U_CHEST)
                        mvs=bodyseg(L_CHEST)%mvs + bodyseg(UPPERARM_L)%mvs + bodyseg(UPPERARM_R)%mvs + bodyseg(HEAD)%mvs
                        bodyseg(seg)%mvs=0.0
                        Tvsadj=0.0
                  case (L_CHEST)
                        mvs=bodyseg(THIGH_L)%mvs + bodyseg(THIGH_R)%mvs
                        bodyseg(seg)%mvs=0.0
                        Tvsadj=0.0
                  case (UPPERARM_L)
                        mvs=bodyseg(FOREARM_L)%mvs
                        bodyseg(seg)%mvs=0.0
                        Tvsadj=bodyseg(FOREARM_L)%Tvs(2)
                  case (UPPERARM_R)
                        mvs=bodyseg(FOREARM_R)%mvs
                        bodyseg(seg)%mvs=0.0
                        Tvsadj=bodyseg(FOREARM_R)%Tvs(2)
                  case (FOREARM_L)
                        mvs=bodyseg(PALM_L)%mvs
                        bodyseg(seg)%mvs=mvs+mblsk+bodyseg(seg)%AVABflow
                        Tvsadj=bodyseg(PALM_L)%Tvs(2)
                  case (FOREARM_R)
                        mvs=bodyseg(PALM_R)%mvs
                        bodyseg(seg)%mvs=mvs+mblsk+bodyseg(seg)%AVABflow
                        Tvsadj=bodyseg(PALM_R)%Tvs(2)
                  case (PALM_L)
                        mvs=bodyseg(L_F1)%mvs+bodyseg(L_F2)%mvs+bodyseg(L_F3)%mvs+bodyseg(L_F4)%mvs+bodyseg(L_F5)%mvs
                        bodyseg(seg)%mvs=mvs+mblsk+bodyseg(seg)%AVABflow
                        Tvsadj=(bodyseg(L_F1)%Tvs(2)+bodyseg(L_F2)%Tvs(2)+bodyseg(L_F3)%Tvs(2)+bodyseg(L_F4)%Tvs(2)+&
                        bodyseg(L_F5)%Tvs(2))/5
                  case (PALM_R)
                        mvs=bodyseg(R_F1)%mvs+bodyseg(R_F2)%mvs+bodyseg(R_F3)%mvs+bodyseg(R_F4)%mvs+bodyseg(R_F5)%mvs
                        bodyseg(seg)%mvs=mvs+mblsk+bodyseg(seg)%AVABflow
                        Tvsadj=(bodyseg(R_F1)%Tvs(2)+bodyseg(R_F2)%Tvs(2)+bodyseg(R_F3)%Tvs(2)+bodyseg(R_F4)%Tvs(2)+&
                        bodyseg(R_F5)%Tvs(2))/5
                  case (THIGH_L)
                        mvs=bodyseg(CALF_L)%mvs
                        bodyseg(seg)%mvs=0.0
                        Tvsadj=bodyseg(CALF_L)%Tvs(2)
                  case (THIGH_R)
                        mvs=bodyseg(CALF_R)%mvs
                        bodyseg(seg)%mvs=0.0
                        Tvsadj=bodyseg(CALF_R)%Tvs(2)
                  case (CALF_L)
                        mvs=bodyseg(FOOT_L)%mvs
                        bodyseg(seg)%mvs=0.0
                        Tvsadj=bodyseg(FOOT_L)%Tvs(2)
                  case (CALF_R)
                        mvs=bodyseg(FOOT_R)%mvs
                        bodyseg(seg)%mvs=0.0
                  ...
                  if (seg==U_CHEST) then
                      ...
                      bodyseg(seg)%Ta(3)=(Qconva+ma*Cp_blood*(Taadj-Ta))*T_STP/Ca+Ta
                      bodyseg(seg)%Tv(3)=(Qconvv+mv*Cp_blood*(Tvadj-Tv)+mperfv*Cp_blood*(coreTemp-Tv)+&
                          mvs*Cp_blood*(Tvsadj-Tv))*T_STP/Cv+Tv
                      end if
                  if (seg==L_CHEST) then
                      bodyseg(seg)%core%coreTemp(3)=(coreMet+shiv-alpha*resp-Qcrsk-Qconva-Qconvv&
                  ...
                  bodyseg(seg)%Ta(3)=(Qconva+ma*Cp_blood*(Taadj-Ta))*T_STP/Ca+Ta
                  bodyseg(seg)%Tv(3)=(Qconvv+mv*Cp_blood*(Tvadj-Tv)+perfusion*Cp_blood*(coreTemp-Tv))*T_STP/Cv+Tv
                  bodyseg(seg)%Tvs(3)=(-Qconvvs+mvs*Cp_blood*(Tvsadj-Tvs)+(mblsk+bodyseg(seg)%AVABflow)&
              ...
              if (bodyseg(seg)%clothed==1) then
                 ...
                 if (ONB==1)then
                 H2Oswt1=H2Oswt1+msw(Tcrav,Tskav,SkinArea,bodyseg(seg)%SWEAT)
                 elseif(ONB==2) then
                 H2Oswt2=H2Oswt2+msw(Tcrav,Tskav,SkinArea,bodyseg(seg)%SWEAT)
                 endif
                 Mresp=1.43e-6*METAo
                 HRexp=0.2*HR_amb+0.0277+6.5e-5*Op_T
                 if (ONB==1) then
                 H2Oexp1=Mresp*(HRexp-HR_amb)
                 elseif (ONB==2) then
                 H2Oexp2=Mresp*(HRexp-HR_amb)
                 endif
            if (seg==25) then
               if (ONB==1) then
               headflux1=(bodyseg(23)%sksens/bodyseg(23)%skin%SkinArea)*0.5+Qress(met,Op_T-273.0,HR_amb)
               trunkflux1=((bodyseg(24)%sksens+bodyseg(25)%sksens)&
                   /(bodyseg(24)%skin%SkinArea+bodyseg(25)%skin%SkinArea))*0.5
               larmflux1=((bodyseg(1)%sksens+bodyseg(3)%sksens+&
               ...
               rarmflux1=((bodyseg(2)%sksens+bodyseg(4)%sksens+&
               ...
               llegflux1=((bodyseg(17)%sksens+bodyseg(19)%sksens+&
                  bodyseg(21)%sksens)&
                   /(bodyseg(17)%skin%SkinArea+bodyseg(19)%skin%SkinArea+&
                   bodyseg(21)%skin%SkinArea))*0.5
               rlegflux1=((bodyseg(18)%sksens+bodyseg(20)%sksens+&
                  bodyseg(22)%sksens)&
                   /(bodyseg(18)%skin%SkinArea+bodyseg(20)%skin%SkinArea+&
                   bodyseg(22)%skin%SkinArea))*0.5

               elseif (ONB==2) then
               headflux2=(bodyseg(23)%sksens/bodyseg(23)%skin%SkinArea)*0.5+Qress(met,Op_T-273.0,HR_amb)
               trunkflux2=((bodyseg(24)%sksens+bodyseg(25)%sksens)&
                   /(bodyseg(24)%skin%SkinArea+bodyseg(25)%skin%SkinArea))*0.5
               larmflux2=((bodyseg(1)%sksens+bodyseg(3)%sksens+&
                  bodyseg(5)%sksens+bodyseg(7)%sksens+&
                  bodyseg(9)%sksens+bodyseg(11)%sksens+&
                  bodyseg(13)%sksens+bodyseg(15)%sksens)&
                   /(bodyseg(1)%skin%SkinArea+bodyseg(3)%skin%SkinArea+&
                   ...
                   bodyseg(9)%skin%SkinArea+bodyseg(11)%skin%SkinArea+&
                   bodyseg(13)%skin%SkinArea+bodyseg(15)%skin%SkinArea))*0.5
               rarmflux2=((bodyseg(2)%sksens+bodyseg(4)%sksens+&
                  bodyseg(6)%sksens+bodyseg(8)%sksens+&
                  bodyseg(10)%sksens+bodyseg(12)%sksens+&
                  bodyseg(14)%sksens+bodyseg(16)%sksens)&
                   /(bodyseg(2)%skin%SkinArea+bodyseg(4)%skin%SkinArea+&
                   bodyseg(6)%skin%SkinArea+bodyseg(8)%skin%SkinArea+&
                      bodyseg(10)%skin%SkinArea+bodyseg(12)%skin%SkinArea+&
                      bodyseg(14)%skin%SkinArea+bodyseg(16)%skin%SkinArea))*0.5
               llegflux2=((bodyseg(17)%sksens+bodyseg(19)%sksens+&
                  bodyseg(21)%sksens)&
                   /(bodyseg(17)%skin%SkinArea+bodyseg(19)%skin%SkinArea+&
                   bodyseg(21)%skin%SkinArea))*0.5
               rlegflux2=((bodyseg(18)%sksens+bodyseg(20)%sksens+&
                  bodyseg(22)%sksens)&
                   /(bodyseg(18)%skin%SkinArea+bodyseg(20)%skin%SkinArea+&
                   bodyseg(22)%skin%SkinArea))*0.5
               endif
...
!       if (time.GE.(sim_time-4*T_STP).and.time.LE.(sim_time))then
        if (ONB==1) then
         ...
         do iin=1,25
            ...
            write(92,50) (ic(iin,r),r=1,5)
  50        format(f9.5," ",f9.5," ",f9.5," ",f9.5," ",f9.5)
!            print*,Op_T-273, ic(iin,1), ic(iin,2)
         end do
         close(unit=92)
         open(197, file='PhysioResults1.csv',status="UNKNOWN",position='append')   
           write(197,*) ic(1,1),',',ic(2,1),',',ic(3,1),',',ic(4,1),',',ic(5,1),',',ic(6,1),',',&
                  ...
                  ic(25,2),',',tavhead1,',',tavtrunk1,',',tavarml1,',',tavarmr1,',',tavlegl1,',',&
                  tavlegr1,',',hrhead,',',hrtrunk,',',hrarml,',',hrarmr,',',hrlegl,',',hrlegr
         close(197)
        elseif (ONB==2) then
         open(unit=94,file="initial_condition2.txt",status="UNKNOWN")
         do iin=1,25
            ...
            ic(iin,4)=bodyseg(iin)%Tv(2)
            ic(iin,5)=bodyseg(iin)%Tvs(2)
            write(94,51) (ic(iin,r),r=1,5)
  51        format(f9.5," ",f9.5," ",f9.5," ",f9.5," ",f9.5)
!            print*, ic(iin,1)
         end do
         close(unit=94)
         open(197, file='PhysioResults2.csv',status="UNKNOWN",position='append')   
           write(197,*) ic(1,1),',',ic(2,1),',',ic(3,1),',',ic(4,1),',',ic(5,1),',',ic(6,1),',',&
                  ic(7,1),',',ic(8,1),',',ic(9,1),',',ic(10,1),',',ic(11,1),',',ic(12,1),',',&
                  ic(13,1),',',ic(14,1),',',ic(15,1),',',ic(16,1),',',ic(17,1),',',ic(18,1),',',&
                  ic(19,1),',',ic(20,1),',',ic(21,1),',',ic(22,1),',',ic(23,1),',',ic(24,1),',',&
                  ...
                  ic(13,2),',',ic(14,2),',',ic(15,2),',',ic(16,2),',',ic(17,2),',',ic(18,2),',',&
                  ic(19,2),',',ic(20,2),',',ic(21,2),',',ic(22,2),',',ic(23,2),',',ic(24,2),',',&
                  ic(25,2),',',tavhead2,',',tavtrunk2,',',tavarml2,',',tavarmr2,',',tavlegl2,',',&
                  tavlegr2,',',hrhead,',',hrtrunk,',',hrarml,',',hrarmr,',',hrlegl,',',hrlegr
         close(197)
        endif

       endif
         if (ONB==1) then
         Qoccsens1=sksenstotal+Qress(met,Op_T-273.0,HR_amb)
         Qocclat1=sklattotal+Qresl(met,Op_T-273.0,HR_amb)
         Tsko_av1=Tskino_av(2)
         Tco_av1=Tcore_av(2)
         Tsk_av1=Tskin_av(2)
         elseif(ONB==2) then
         Qoccsens2=sksenstotal+Qress(met,Op_T-273.0,HR_amb)
         Qocclat2=sklattotal+Qresl(met,Op_T-273.0,HR_amb)
         Tsko_av2=Tskino_av(2)
         Tco_av2=Tcore_av(2)
         Tsk_av2=Tskin_av(2)
         endif
      end do
